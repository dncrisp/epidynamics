function [isix,isiy,ampx,ampy,pks,locs] = isi_amp_offsetAnalysis(y,Fs,start,stop,thresh,diffthresh,ampwin)
...
%
%isix: The times of the ISI. Time reversed for equation fitting.
%
%ampy: The peak-to-peak amplitude of the spikes.
%
%ampx: The times of the spikes. Time reversed for equation fitting.
%
...
isiy = diff(locs2);
blocs = abs(locs2 - locs2(end)); % Reverses time for equation fitting
isix = blocs(2:end) + 0.1; % Assumption that last ISI point occurs 0.1 second away from seizure end.
ampx = blocs + 0.1;
locs = locs + start;
for i = 1:length(pks)
    t_a = locs(i) - ampwin/2; 
